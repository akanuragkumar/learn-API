from api_schema import unit_response_schema as response_schema
import pymysql
import config
...
def get_data_from_db(code):
	cursor = connect_db()
	query = f"SELECT *  FROM {config.DbDetails.db_name}.{config.TableNames.knowledge_vectors} where code='{code}' and type='units';"
...
def fill_learning_map(learnpath_code):
	goal_code, exam_code, subject_code, unit_code = learnpath_code.split("--")
